...
import com.example.mebelcart.databinding.FragmentAddressBinding
import com.example.mebelcart.util.Resource
import com.example.mebelcart.viewmodel.AddressViewModel
...
class AddressFragment: Fragment() {
        ...
        lifecycleScope.launchWhenStarted {
            viewModel.deleteAddress.collectLatest {
                when (it) {
                    ...
                    is Resource.Success -> {
                        binding.progressbarAddress.visibility = View.INVISIBLE
                        findNavController().navigateUp()  // Close after deletion
                    }
                    is Resource.Error -> {
                        binding.progressbarAddress.visibility = View.INVISIBLE
                        Toast.makeText(requireContext(), it.message, Toast.LENGTH_SHORT).show()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        // Save button logic (Add or Update based on args)
        binding.apply {
